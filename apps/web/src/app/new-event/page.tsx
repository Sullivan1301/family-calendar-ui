    ...
    try {
      const response = await fetch('/api/events', {
        ...
        body: JSON.stringify({
          ...
          location: formData.location || activeLoc || undefined,
        }),
      });
      ...
      // Rediriger vers la page des événements ou le dashboard
      setTimeout(() => {
        router.push('/events');
      }, 1000);
    } catch (error: any) {
    ...
    <div className="animate-fade-in pb-16">
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 gap-4">
        ...
        <button 
          ...
          className="btn-primary flex items-center gap-2 py-3 px-8 shadow-xl disabled:opacity-50 disabled:cursor-not-allowed"
        >
          <Save size={18} />
          <span>{isSubmitting ? 'Enregistrement...' : 'Enregistrer l\'événement'}</span>
        </button>
      </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div className="sm:col-span-2 space-y-2">
                  ...
                  <div className="input-with-icon">
                    ...
                    <input 
                      type="text" 
                      className="flex-1 bg-transparent border-none outline-none text-sm font-semibold text-tba-blue placeholder:text-tba-gray-light" 
                      placeholder="Ex: Anniversaire de Maman, Vacances d'été…" 
                ...
                <div className="sm:col-span-2 space-y-2">
                  ...
                  <textarea 
                    className="w-full px-4 py-3 rounded-xl bg-white border border-tba-border text-sm font-semibold text-tba-blue focus:border-tba-red outline-none min-h-[120px] resize-none transition-all placeholder:text-tba-gray-light" 
                    placeholder="Décrivez l'événement, les détails pratiques…"
